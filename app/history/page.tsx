...
export default function HistoryPage() {
  ...
  const fetchHistory = async (token: string) => {
    try {
      ...
      // map ข้อมูลจาก DB ให้ตรงกับ HistoryItem
  ...
  const getStatusBadge = (item: HistoryItem) => {
    ...
    if (item.status === "borrowed") {
      ...
      return <Badge className="bg-blue-100 text-blue-800">ยืมอยู่</Badge>
    }
  ...
  const handleReturn = async (borrowRecordId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // update state แทนการ fetch ใหม่
        setHistory((prev) =>
          prev.map((item) =>
          ...
          <CardContent>
            {history.length === 0 ? (
              ...
              <div className="space-y-4">
                {history.map((item) => (
                  <div key={item._id} className="border border-gray-700 rounded-lg p-4 hover:bg-gray-800 transition-colors">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex-1">
                        <h4 className="font-medium text-lg mb-2 text-white">{item.equipmentName}</h4>
